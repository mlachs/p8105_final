...
NPS_data=data %>% unnest(data) %>% unnest(activities,names_sep = '_') %>% unnest(topics,names_sep = '_') %>% 
  janitor::clean_names() %>% 
  mutate(
...
combined_data <- visitation_data %>%
  right_join(NPS_data %>% select(park_code, full_name, latitude, longitude),
            by = c("park_code"))
...
regression with entrance fees, activities, park_type ?
