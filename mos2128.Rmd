...
NPS_data=data %>% unnest(data) %>% select(fullName,description,latitude,longitude,activities,states,entranceFees,operatingHours,activities, designation, parkCode, topics) %>%  unnest(activities, names_sep = '_') %>% unnest(entranceFees,names_sep = '_') %>% unnest(operatingHours,names_sep = '_') %>% unnest(topics, names_sep = "_") %>%  janitor::clean_names() %>% 
  mutate(
    latitude = as.numeric(latitude), 
    longitude = as.numeric(longitude),
    activities_name = factor(activities_name), 
    topics_name = factor(topics_name)
  )
```
...
model to predict visitation based on entrance fees, activities, park_type ?
